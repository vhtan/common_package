...
public struct Configuration {
    ...
    public var environment: Environment {
        do {
            return try Environment(value: Configuration.value(for: "ENVIRONMENT"))
        } catch { }
    ...
    public var baseAPIURL: URL {
        do {
            let path = try Configuration.value(for: "BASE_API_URL")
    ...
    public var fileAPIURL: URL {
        do {
            let path = try Configuration.value(for: "FILE_API_URL")
            if environment.isDebug {
                return URL(string: "http://\(path)")!.appendingPathComponent("v1")
            } else {
                return URL(string: "http://\(path)")!.appendingPathComponent("v1")
